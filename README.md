...
💡 运行逻辑说明：
脚本每天 UTC 0:20 (北京时间 8:20) 自动唤醒。唤醒后会计算当前日期与 START_DATE 的相差天数。只有相差天数是 4 的倍数（如 0天, 4天, 8天...）时，才会真正执行续期任务；否则直接跳过。

🌐 代理格式支持
如果你配置了 NODE_LINK，请确认你在本地 v2rayN 等软件中测试该节点是可用的。目前支持以下代理协议的完整分享链接：
